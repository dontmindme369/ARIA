...
CORPUS_DIR = ROOT / "training_data" / "conversation_corpus"
RAG_RUNS_DIR = ROOT / "rag_runs" / "aria"
...
class WatcherState:
    ...
    def __init__(self) -> None:
        ...
        self.lock = threading.RLock()  # Use RLock for reentrant locking (save_state called from locked methods)
        self.load_state()
...
def extract_message_content(message: Dict[str, Any]) -> str:
    ...
    Structure: message → versions[] → content[] → text
    ...
    try:
        ...
        for user_query, assistant_response, message_id in new_pairs:
            ...
            if pack_match:
                ...
                logger.info(f"✅ ARIA Triple captured")
                ...
                logger.info(f"✅ Conversation captured (no pack)")
...
class ConversationHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event: FileSystemEvent) -> None:
        ...
        if captured > 0:
            logger.info(f"🔔 Event captured {captured} new entry(ies)")
    ...
    while not stop_event.is_set():
        try:
            ...
            if conv_files:
                ...
                if total_captured > 0:
                    logger.info(f"🔔 Polling captured {total_captured} new entry(ies)")
...
def main() -> int:
    ...
    try:
        observer.start()
        logger.info("🌀 Observing ALL conversations... (Ctrl+C to stop)")
        logger.info("")
