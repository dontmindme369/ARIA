...
class SourceReliabilityScorer:
    ...
    - Prior: Beta(ÃŽÂ±, ÃŽÂ²) distribution over success rate
    ...
    def _compute_reliability(self, source: SourceStats) -> float:
        ...
        - Posterior: Beta(ÃŽÂ± + successes, ÃŽÂ² + failures)
    ...
    def get_average_reliability(self) -> float:
        ...
        Convenience method for quick access to reliability metric
        
        Returns:
            Average reliability score (0.0 to 1.0)
        """
        if not self.sources:
            return 0.5  # Neutral default
        
        all_reliability = [s.reliability_score for s in self.sources.values()]
        return float(np.mean(all_reliability))
    
    @property
    def avg_reliability(self) -> float:
        """Property alias for get_average_reliability() for backwards compatibility"""
        return self.get_average_reliability()
    ...
    def _load_state(self):
        ...
        try:
            ...
            print(f"Ã¢Å“â€œ Loaded source reliability: {len(self.sources)} sources tracked")
