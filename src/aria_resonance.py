...
class ResonanceDetector:
    ...
    def _extract_resonance_pattern(self) -> Dict[str, Any]:
        ...
        recent = [
            m
            for m in list(self.resonance_history)[-self.stable_threshold :]
        ...
        pattern = {
            ...
            "measurements": {
                ...
                "avg_exemplar_fit": statistics.mean(
                    [m["exemplar_fit"] for m in recent]
                ),
    ...
    def _aggregate_query_features(
        ...
        agg = {
            ...
            "avg_word_count": statistics.mean(
                [f.get("word_count", 0) for f in features_list]
            ),
        }
...
class SelfTuningEngine:
    ...
    def extract_tuning_recommendations(
        self, current_config: Dict[str, Any], query_features: Dict[str, Any]
        ...
        if "preset" in best_config and best_config["preset"] != current_config.get(
            "preset"
        ):
        ...
        for param in ["top_k", "sem_limit", "rotations", "rotation_subspace"]:
            if param in best_config:
                ...
                if current_val and abs(current_val - recommended_val) > (
                    current_val * 0.15
                ):
    ...
    def _find_similar_patterns(
        self, query_features: Dict[str, Any], patterns: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for pattern in patterns:
            ...
            if pattern_length > 0:
                ...
                if 0.5 <= length_ratio <= 2.0:
                    ...
                    if (is_question and pattern_q_ratio > 0.5) or (
                        not is_question and pattern_q_ratio <= 0.5
                    ):
    ...
    def apply_tuning(
        self, current_config: Dict[str, Any], recommendations: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class MetaPatternExtractor:
        ...
        for seq in self.query_sequences:
            ...
            for i in range(len(seq) - 1):
                ...
                if mode1 and mode2 and mode1 != mode2 and res2 > res1:
                    transitions.append(
                        {"from": mode1, "to": mode2, "resonance_gain": res2 - res1}
                    )
    ...
    def _detect_iterative_refinement(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "avg_gain": statistics.mean(
                [s["resonance_gain"] for s in refinement_sequences]
            ),
            "success_rate": len(
                [s for s in refinement_sequences if s["final_resonance"] > 0.85]
            )
            / len(refinement_sequences),
    ...
    def _detect_followup_effectiveness(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "improvement_rate": sum(1 for f in followups if f["improved"])
            / len(followups),
            "avg_resonance_change": statistics.mean(
                [f["resonance_after"] - f["resonance_before"] for f in followups]
            ),
        }
