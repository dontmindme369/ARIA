...
class QualityFilter:
    ...
    def is_pdf_garbage(self, text: str) -> bool:
        ...
        marker_count = sum(
            1 for tok in tokens if tok.lower() in self.PDF_GARBAGE_MARKERS
        )
        if marker_count >= 2:
            return True
        ...
        if re.search(
            r"(?:\\x[0-9A-Fa-f]{2}|#[0-9A-Fa-f]{6}|<[0-9A-Fa-f\\s]{16,}>)", sample
        ):
            return True
    ...
    def is_low_quality(self, text: str, path: str = "") -> bool:
        ...
        # Too much non-alphabetic content (very lenient)
        if self._alpha_ratio(text) < 0.15:
            return True
    ...
    def is_on_topic(self, text: str) -> bool:
        ...
        if self.relax_must and self.must_terms:
            ...
            for term in self.must_terms:
                term_parts = [
                    p
                    for p in re.findall(r"[a-z][a-z0-9\\-]{2,}", term)
...
class DiversityEnforcer:
    ...
    def enforce(
        self, items: List[Dict[str, Any]], keep_at_least: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        for source in by_source:
            by_source[source].sort(key=lambda x: x.get("score", 0.0), reverse=True)
...
class PackStats:
    ...
    def compute(self, items: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Chunk lengths
        chunk_lengths = [len(self._get_text(it)) for it in items]
...
class ARIAPostfilter:
    ...
    def filter(
        ...
        removal_reasons = {"quality": 0, "topic": 0, "glob": 0}
        ...
        if len(filtered) < self.min_keep:
            # Add back top items regardless of filters
            all_scored = sorted(items, key=lambda x: x.get("score", 0.0), reverse=True)
    ...
    def _should_drop_glob(self, path: str) -> bool:
        ...
        return any(fnmatch.fnmatch(path, glob) for glob in self.drop_globs)
...
def main():
    ...
    # Parse filters
    must_terms = (
        [t.strip() for t in args.must_terms.split(",")] if args.must_terms else None
    )
    drop_globs = (
        [g.strip() for g in args.drop_globs.split(",")] if args.drop_globs else None
    )
    ...
    if args.output:
        output_pack = {
            "items": result["items"],
            "meta": {**pack.get("meta", {}), "postfilter_stats": result["stats_after"]},
        }
